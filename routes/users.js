...
const GET_INVOICES =
  "SELECT invoice_id,grand_total,buyer_id,seller_id,status,created_at FROM invoice";
...
module.exports = (app) => {
  ...
  app.get("/users", function (req, res) {
    db.query("SELECT * FROM users", function (err, results) {
      if (err) res.send({ message: "err to fetch users" });
    ...
    db.query(check_user, (err, results) => {
      if (err) return res.send(err);
        ...
        db.query(GET_INVOICES, (err, results) => {
          if (err) return res.send(err);
          else if (results && results.rows) return res.send(results.rows);
        });
